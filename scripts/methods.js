function mousePressed() { //when the mouse is pressed
  ...
  if (loaded >= 3) { //if both the JSON and the background are loaded
    for (var i = 0; i < countries.length; i++) { //loop through the countries
      if (collidePointPoly(mouseX, mouseY, countries[i].path)) { //if the country has been clicked
        box = prompt("You have clicked " + countries[i].name +
        ". Enter a team name to invade, or build one of the following - \"wall\", \"hippo\", or \"croc\" by entering it in the box below.", ""); //get an instruction
  ...
  if (!clicked) { //if we didn't click a contry
    ...
    if ((overColor.levels[0] == oceanColor.levels[0]) &&
    (overColor.levels[1] == oceanColor.levels[1])) { //if we clicked an ocean
      box = prompt("You have clicked the ocean, to make a boat, enter a team name",
      ""); //get an instruction
      clicked = true; //we have clicked the ocean
      if (box) {
        things.push(new Thing(mouseX, mouseY, whichOwner(box), "boat"));
        //change owners
      }
    }
  }
